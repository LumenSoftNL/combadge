...
class ESPNowPacket {
 public:
  ESPNowPacket(uint64_t mac_address, const uint8_t *data, int len) {
  ...
  uint64_t mac_address() { return bssid_; }
  std::vector<uint8_t> data() { return data_; }

  uint8_t get_counter() { return send_cound_; }
  void inc_counter() { send_cound_ = send_cound_ + 1;}
  void reset_counter() { send_cound_ = 0; }

  void broadcast(bool value) { this->broadcast_ = value; }
  bool broadcast() const { return }
...
class ESPNowComponent : public Component {
 public:
  ...
  void send_packet(const ESPNowPacket packet) {
    global_esp_now->send_queue_.push(std::move(packet));
  }
...
template<typename... Ts> class SendAction : public Action<Ts...>, public Parented<ESPNowComponent> {
 public:
  ...
  void set_mac_address(uint64_t mac_address) {
    this->mac_address_ = mac_address;
  }
