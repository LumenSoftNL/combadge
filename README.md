...
---- |   |   |
0x33 | start sending voice                      | client to client |
0x3f | Voice package send                       | server/client to client/server |
0x35 | stop sending voice                       | client to client |
---- |   |   |
0x3f | Close call                               | client to client |
---- |   |   |
0x40 | Client peered on extern network          | network to server |
0x4f | Client left from extern network          | network to server |
---- |   |   |
0xe0 | Start update firmware                    | server to client | size of file, checksum of file
0xe1 | Upload update firmware                   | server to client | package counter, fileblock
---- |   |    |
0xff | Alarm help needed.                       | client to broadcast |


### *0x01* Ping the server

### *0x02* Pong from server

### *0x03* Reguest client details (new in network)

### *0x04* Guest use request

### *0x05* New device

### *0x07* Accept new client

### *0x0d* Update username

### *0x0e* Update owner IP (used in other network)

### *0x10* Ack & *0x11* Nack response

### *0x30* Start call request

### *0x31* Deliver other mac address

### *0x33* start sending voice

### *0x34* Send Voice package

### *0x35* stop sending voice

### *0x39* Close call

### *0x40* Client peered on extern network

### *0x4f* Client left from extern network

### *0xe0* Start update firmware

### *0xe1* Upload update firmware
